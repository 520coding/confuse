...
1. 新增通用功能‘版本迭代混淆’，过审后迭代更新，沿用上一次（也可以任意选择版本）混淆记录增量混淆，保持版本连续性，模拟正常开发。
   1. 优势：只要维护源码，正常开发流程，避免在已过包的混淆项目上继续开发，做到开发和混淆同步且各自独立
   1. 目前80%的主要功能均支持更新混淆，后续将剩余补上
2. 修复OC[重命名多语言]，项目中没有多语言的时候出现的bug
2. 修复V3.6.0版本因为调换OC[插入文件]执行顺序，导致小概率出现插入文件名重复的bug
2. 修复测试工程全部勾选情况下，小概率出现C++漏改一个了方法名
2. 修复OC[插入属性]百分比控制为100%时，属性名和类名同名极端情况，导致类名被改的bug
