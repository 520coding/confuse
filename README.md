...
19. 优化中...~~[插入文件]，插入ViewController类文件，相互调用及源文件调用，支持自动、收入导入项目~~
19. [插入图片]，类中自动初始化、调用及销毁
19. 优化中...~~[插入文本]，文件（json、txt、doc）~~
19. [重命名类]，可指定添加前缀，优势：
    1. 智能名词替换
    1. 可设置‘重命名同名文件’
    1. 可设置‘重命名相似字符串’，(忽略|相等|包含)三种设置
24. [修改文件属性]，如创建时间、访问时间、修改时间
...
1. 重构[插入属性]
   1. 移除原来固定插入类型及简单的alloc init delloc，升级为关联已有类型
   1. 移除随机拼接单词，使用智能名词替换
   1. 新增‘百分比控制’开关
   1. 新增‘Model后缀’开关，目的：避免Model归档或者数据转模型失败
