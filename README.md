...
18. [插入属性]，类中自动初始化、调用及销毁
18. [插入图片]，类中自动初始化、调用及销毁
18. 优化中...~~[插入文本]，文件（json、txt、doc）~~
18. [重命名类]，可指定添加前缀，优势：
    1. 智能名词替换
    1. 可设置‘重命名同名文件’
...
1. 重构[重命名图片]
   1. 移除图片名限制条件，理论上可重命名所有图片
   1. 移除随机拼接单词，使用智能名词替换
   1. 移除图片位置限制，现在图片位置不局限xxx.xcassets文件中，可任意位置
   1. 新增‘运行拼接名称’开关，用于运行时通过字符串拼接生成的图片名
   1. 新增‘重命名关联字符串’开关，用于修改字符串与图片名相等的情况，有一定风险，自己看情况使用
   1. 新增‘忽略危险名称’开关，名称长度<4为危险名称，注意：一个汉字长度为3
   1. 自动纠正图片名和xcassets文件夹名不对应的情况（实际开发基本保持一致）
