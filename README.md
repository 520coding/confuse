...
1. 新增C++[重命名类]，支持模板等类型
   ...
   1. 前缀设置
   1. 可设置‘重命名同名文件’
2. 支持cocos2dx自动过滤，避免重新扫描
2. 新增所有耗时较长的地方的进度提示信息，避免在大项目中造成卡死假象
2. 新增日志输出重命名系列的修改前后名称对比映射
2. 优化头文件‘#include "../BaseTest.h"’包含..导致的漏改
2. 优化代码，整体性能提升，减少混淆时间10%~40%（后续将推出快速混淆模式）
2. 优化APP功能信息展示等
