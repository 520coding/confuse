...
1. Objective-C，重构的目的是为了提高工具的通用性和稳定性，及强化功能
   ...
   2. 重构《多语言》
   2. 移除混淆前需要创建目录Confuse、Discard要求（这部分是老代码，需要些时间适配【已完成90%】，请见谅）
...
1. 重构[修改工程]
   1. 移除bundleID、Version混淆，原因：该部分改动简单且主要由用户自定义
   1. 新增‘修改UUID’，所有xx.xcodeproj的uuid都将被修改，如需过滤，请在黑名单中设置
   1. 新增‘修改target’，所有自定义target名称，及相关联的内容、文件名等都将被修改
2. 优化xib、storyboard修改时有可能改变视图层级关系导致视图被盖住
2. 新增‘敏感词’自定义
2. 移除[重命名多语言][插入文件][插入文本]，原因：该三个功能与其他工具类似，个人认为该方式已过时，且与新版[修改工程]冲突，故移除，后续将重构
2. 移除项目基本配置的‘其他target名称’设置
2. 优化代码，减少混淆前期时间25秒左右
