...
14. [插入属性]，类中自动初始化、调用及销毁
14. [插入图片]，类中自动初始化、调用及销毁
14. 优化中...~~[插入文本]，文件（json、txt、doc）~~
14. [重命名类]，类名及对应的文件名一起修改，**支持近似替换**，可指定添加前缀
14. [修改项目]，基本配置信息，例如：版本号、SDK的BundleID
...
1. Objective-C，重构的目的是为了提高工具的通用性和稳定性，及强化功能
   1. ~~弃用所有的随机单词无脑组合~~，使用（项目原有词汇+相近替换+类型相关+部分旧词汇随机组合）
...
1. 优化[修改方法]
   1. 提升改功能混淆速度
   1. 优化模板，由原理的单模板拆分为双模板，加强与类型的关联度
   1. 修复方法的返回值类型是数值类型且返回时引用了self.属性情况下混淆后类型丢失问题
2. 优化[修改局部变量]
   1. 提升改功能混淆速度
   1. 优化模板，由原理的单模板拆分为双模板，加强与类型的关联度
3. 优化[重命名属性]
   1. 新增<前缀>自定义设置
   1. 修复特殊情况下的关联outlet修改错误
4. 优化[重命名方法]，新增<前缀>自定义设置
4. 优化[重命名字符串]
   1. 新增<最少长度>自定义设置
   1. 新增<有效个数>自定义设置
6. 优化[布局偏移]
   1. 支持SDAutoLayout
   1. 修复单词拼写错误Mansonry->Masonry
