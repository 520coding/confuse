...
1. [重命名图片]，智能名词替换，自动纠正图片名和xcassets文件夹名不对应的情况
   ...
   1. 可设置‘忽略危险名称’开关
6. [重命名属性]，支持@property的所有类型，基本功能，改名字、前缀类似其他工具，不做过多描述，优势：
...
2. C++，现有功能还不具备通用性，暂时不开放，准备重构中...
   1. 字符串加密混淆
   1. 方法，重命名，（完成50%，暂停，准备把OC部分做全）、插入、修改
   1. 属性，重命名、修改、插入
...
1. 更新[修改图片]
   1. 移除原来修改图片只能在xcassets目录限制
   1. 移除原来图片名或图片文件夹不能是空格、中文等限制
   1. 优化‘质量调整’‘偏移范围’
   1. 新增‘像素点数’开关，实现局部修改图片
2. 修复上一个版本预检查功能失灵bug
2. 优化环境检查针对‘未安装Homebrew’的提示信息不合理问题
