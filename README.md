...
<a name="VPIgN"></a>
# 自述
其实识别一个工具的优劣，只需看看它能否修改方法名的所有参数名（极少）、带block的参数的方法（极少），修改布局（较少）。更别说“还有谁...”能识别宏、区分继承链等上下文关联内容。也欢迎大家使用不同工具混淆测试工程[**confuse_test**](https://github.com/520coding/confuse/tree/master/confuse_test)，对比效果。<br />马甲包的本质：
...
10. [修改局部变量]，模拟人工封装调用，变量名关联类型，优势：
    ...
    1. 可多次执行，指数x2递增
13. [修改字符串]，加密处理（硬编码->内存），原始字符串保留在注释中方便检查
    1. 设置‘最少长度’过滤
    1. 也可设置‘有效个数’搭配使用
...
20. [插入文件]，生成其它文件（创建自定义控件，模拟正常开发），项目中自动调用；**注意：目前需要手动导入文件**(在项目根路径下，会生成"**other_***"的文件夹，将它拖入工程即可）
20. 优化中...~~[重命名多语言]，支持汉字，所有字符串将被修改~~
<a name="GTE99"></a>
### C++

1. [重命名方法]，近似Xcode的Rename功能，【智能识别不可修改部分】
   1. 语法相关，识别类型、模板、重载、重写、继承等关系
   1. 【智能名词替换】，混淆后不同类存在相同方法名，模拟正常开发
2. [重命名属性]，支持所有类型属性，【智能识别不可修改部分】
...
1. Objective-C，重构的目的是为了提高工具的通用性和稳定性，及强化功能
   1. [插入文本]，目标逼近正常开发，让插入的垃圾不在是垃圾，告别所谓的垃圾
   1. 重构《多语言》
   1. 音频、视频文件使用少，后续添加
2. C++，开发中...
   1. 字符串加密混淆
...
1. 新增C++[重命名属性]，支持所有类型属性，【智能识别不可修改部分】
   1. 识别语法，识别类型、继承，同名属性不同类混淆后将不一致
   1. 【智能名词替换】，混淆后不同类存在相同属性名，模拟正常开发
2. 优化初次安装环境，跳过HomeBrew的自动更新
2. 优化[项目配置]，允许target包含符号‘-’
2. 优化[修改字体]、[修改颜色]、[布局偏移]，加快混淆速度，优化混淆效果，支持Masonry的MAS_SHORTHAND、MAS_SHORTHAND_GLOBALS模式
2. 修复特殊情况[环境检查]中OC属性检查终止运行bug
2. 修复OC[重命名属性]特殊情况下有可能混淆后同名，正常项目基本不会出现，可忽略
