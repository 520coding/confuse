...
<a name="fad5222c"></a>
## 已完成
_以下功能均支持黑名单（二级）过滤，自由控制混淆内容，几乎适应所有项目。_
<a name="Pfu38"></a>
### 通用部分
...
3. [修改字符串]，支持任意字符串，加密处理（硬编码->内存），原始字符串保留在注释中方便检查
   1. 设置‘最少长度’过滤
   1. 也可设置‘有效个数’搭配使用
5. [重命名类]，支持模板等类型
   1. 可切换旧模式
...
1. 新增C++[修改方法]，利用重载技术修改函数原型并调用修改形参
1. 新增部分进度提示信息，避免大项目卡顿假象
1. 优化OC[重命名图片]，大项目中预估再减少混淆时间20%~30%
1. 优化OC[修改图片]，升级为通用功能
1. 优化通用功能，使其完全独立，减少混淆时间，目前可当成小工具使用，基本适用所有iOS项目
1. 修复上一个版本呢‘Scheme’名称无法自动识别
