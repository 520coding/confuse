...
<a name="GZrkm"></a>
# 前言
因公司发展需要，本人19年中旬开始从事iOS[马甲包业务](https://www.yuque.com/docs/share/7e70244c-5dea-4035-b634-65cc082097da?#《马甲包简介》)，前期也使用过目前市面上其他得工具，实际效果不太理想。经过大量实践，开发出一款功能齐全的[混淆工具](https://github.com/520coding/confuse)。工具的主要功能OC、C++已封装成Mac应用，其他功能还在封装中，敬请期待。目前公测阶段: _**免费**_
<a name="qPY4i"></a>
# 提示
...
19. [插入文件]，生成其它文件（创建自定义控件，模拟正常开发），项目中自动调用；**注意：目前需要手动导入文件**(在项目根路径下，会生成"**other_xxx_file**"的文件夹，将它拖入工程即可）
19. [插入文本]，生成json、txt、doc、plist等文本文件，项目中自动调用；**注意：目前需要手动导入文件**(在项目根路径下，会生成"**other_xxx_text**"的文件夹，将它拖入工程即可）
19. 优化中...~~[重命名多语言]，支持汉字，所有字符串将被修改~~
<a name="GTE99"></a>
### C++
...
<a name="NVIkj"></a>
### Cocos2d-x
该部分功能整合至C++中，支持cocos2dx自动过滤<br />_以上所有功能均支持黑名单过滤，对指定的内容进行屏蔽，忽略混淆。_
<a name="cf7Dx"></a>
### 名词解释
...
1. 新增C++[修改字符串],加密处理（硬编码->内存），原始字符串保留在注释中方便检查
   1. 设置‘最少长度’过滤
   1. 也可设置‘有效个数’搭配使用
2. 新增局部变量模板支持类型NSDictionary
2. 优化OC[修改字符串]，减少混淆时间20%~30%，没有字符串时不处理，避免插入空内容
2. 更新‘target选项’为‘scheme选择’，移除名称限制，现在可任意
2. 修复上一个版本bug，原因：上一个版本修改太多了，改懵了，抱歉
   1. OC[插入文件]混淆失效
   1. OC[重命名方法]init开头混淆异常
