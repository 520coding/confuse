...
目前C++和OC混合项目的C++默认不修改，xx.mm的文件可混淆，正则完善中。<br />目前Swift和OC混合项目的Swift默认不修改，但是Swift引用到OC的需要自己添加值黑名单，后续将优化。
<a name="VPIgN"></a>
# 自述
...
22. 优化中...~~[重命名多语言]，支持汉字，所有字符串将被修改~~
22. [修改文件属性]，如创建时间、访问时间、修改时间
22. [修改项目]，无需删除Cocoapods
    1. 可设置‘修改uuid’，彻底翻新
...
- 智能识别不可修改部分：通过类型及继承链方式识别系统、第三方、Pod方法，并不是‘简单’的相等判断，例如：
   - + (void)init;原则上任何地方都能改
   - - (void)reloadData;不是UITableView的子类是可以改的
   - @property (readonly) NSUInteger length;如果不是NSString的子类也是可以改的
<a name="OEesy"></a>
### 规划中
更新迭代将按照以下顺序依次进行
...
1. Objective-C，重构的目的是为了提高工具的通用性和稳定性，及强化功能
   1. 优化插入垃圾，目标逼近正常开发，让插入的垃圾不在是垃圾，告别所谓的垃圾
      - [插入文件]【已完成70%】，提取项目原有信息，进行合理组合并创建类，然后在源文件调用，支持自动、收入导入项目
...
1. 修复[修改字体]、[修改颜色]、[布局偏移]前两个版本功能开关失效问题，原因：结构设计不合理导致
1. 优化[项目基本配置]
   1. 移除‘xcodeproj路径’设置，改为自动
   1. 移除‘Pods路径’设置，自动识别
   1. 移除‘混淆路径’设置，无需设置Confuse文件夹，省去修改项目结构，提高通用性
   1. 优化‘忽略路径’，无需设置Discard文件夹，省去修改项目结构，支持文件(夹)多路径选择（支持正则），可预搜索查看实际忽略结构，防止选错忽略路径
   1. 优化‘主要target名称’，更名为‘target’。支持自动扫描及默认选择第一个target，用户可以在扫描列表中选择，省去手动输入
   1. 优化提示信息，清晰流程
3. 新增遇到没有权限文件（例如：只读文件）提示，以免造成卡死假象
3. 新增C++的初步兼容模式，识别.mm文件
3. 更新UI，新增OC、C++、Swift开关，过度阶段
