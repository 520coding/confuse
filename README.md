...
2. [Insert Picture], which automatically inserts pictures, and simulates manual calls according to the context and type, and the number of insertions can be specified
2. [Rename enum], support associated values and primitive values
2. [Rename attribute], basic function, without too much description, advantages:
   ...
   1. File name Model suffix filtering can be set
5. [Insert property], use calculated properties to wrap and call and replace the original properties
   1. Support storage and calculation of attributes, observers, wrappers, class attributes
   1. File name Model suffix filtering can be set
6. [Rename method], the basic functions are renamed similar to other tools, without too much description, advantages: identification of inheritance chain nested types, support for (class, struct, enum) static methods and instance methods, and optional chains, etc.
6. [Insert method], encapsulate the return value of the original method and use the context to call additional associated methods, saying goodbye to "garbage code"
6. [Modification method], use overloading technology to modify the function prototype and call the modified parameter
6. [Modify font] , randomly fine-tune the font used in the project, and identify macros
6. [Modify color], randomly shift the color of UI controls in the project
6. [UI layout offset], temporarily only supports Frame layout fine-tuning
6. [Insert local variable], split single-line compound calls, change the execution order
6. [Modify local variable], simulate manual encapsulation call, variable name association type (support nesting), advantages:
...
1. Removed OC [Delete Comment], changed it to the public function [Delete Comment], identified single-line and multi-line comments, removed sub-options to simplify the operation steps
1. Optimize the interaction, update the configuration operation, and avoid the freezing of switching projects
1. Fix 133 exceptions in special cases
