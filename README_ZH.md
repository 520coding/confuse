...
4. [修改方法]，利用重载技术修改函数原型并调用修改形参
4. [修改字符串]，识别单行、多行、字符串插值、及扩展字符串，改后由加密和拆分字符组等多种方式自由组合，并保留原有字符的注释，方便检查
   1. 设置‘最少长度’过滤
   1. 也可设置‘有效个数’搭配使用
7. [重命名类]，类名不限制（例如：my、My），识别嵌套类型及typealias，支持class、struct，enum、protocol
   1. 可设置‘重命名同名文件’
   1. 可设置‘前缀’
...
1. 新增Swift[插入属性]，利用计算属性对原属性包装及调用替换
   1. 支持存储和计算属性、观察器、包装器、类属性
   1. 可设置文件名Model后缀过滤
2. 优化[修改字符串]，解决转义字符串混淆后解密后多转义一次问题
2. 优化OC[插入文件]，优化性能减少混淆时间
2. 修复重置配置后[环境检查]变成英文
2. 修复切换账号相关信息没有及时刷新问题
