...
26. [重命名类]，类名不限制（例如：my、My），可指定添加前缀，支持class及分类名、struct、protocol，优势：
    1. 智能名词替换
    2. 可设置‘重命名同名文件’
...
# 更新日志
<a name="XFWnf"></a>
### v7.7.2（2024.06.29）热更新

1. 修复OC[修改控制流]，对NSAssert和条件语句换行的错误处理
2. 强化Swift[重命名属性]，支持SmartCodable和自定义映射补全
3. 优化Swift[修改字符串]，短的字符串混淆改变少的问题
4. 模板的Bool默认不再展开，降低代码量
