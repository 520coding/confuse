...
2. [重命名属性]，基本功能，不做过多描述，优势：
   ...
   1. 可设置文件名Model后缀过滤
5. [插入属性]，利用计算属性对原属性包装及调用替换
   1. 支持存储和计算属性、观察器、包装器、类属性
   1. 可设置文件名Model后缀过滤
6. [重命名方法]，基本功能改名字类似其他工具，不做过多描述，优势：识别继承链嵌套类型，支持（class、struct、enum）的静态方法和实例方法，及可选链等
6. [插入方法]，封装原方法的返回值并利用上下文调用额外关联方法，告别“垃圾代码”
6. [修改方法]，利用重载技术修改函数原型并调用修改形参
6. [修改字体]，对项目中使用的字体随机微调
6. [修改颜色]，对项目中UI控件颜色随机偏移
6. [UI布局偏移]，暂时只支持Frame布局微调
6. [插入局部变量]，拆分单行复合调用，改变执行顺序
6. [修改局部变量]，模拟人工封装调用，变量名关联类型（支持嵌套），优势：
...
1. 移除OC[删除注释]，改为公共功能[删除注释]，识别单行、多行注释，移除子选项简化操作步骤
1. 优化交互，更新配置操作，避免切换项目卡顿
1. 修复特殊情况下133中断异常
