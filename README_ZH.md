...
1. 优化Swift[重命名属性]，修复和局部变量名称重复的时候，缺少self.调用
2. 优化Swift[重命名方法]，有可能和父类修改不同步问题，优化‘参数标签’的兼容性
3. 优化Swift[修改方法]，形参隐式可选类型的时候，缺少类型声明，以及泛型函数支持
4. 优化Swift[插入方法]，避免跨行链式调用错误封装
5. 新增非法字符文件报错主动提示
6. 优化Swift的模板个别错误语法
